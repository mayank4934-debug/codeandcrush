...
export default function VideoPlayerPage({
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      {/* Top bar */}
      <div
        className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-border bg-card/95 backdrop-blur shrink-0"
      ...
      <div className="flex-1 overflow-hidden flex flex-col lg:flex-row min-h-0">
        {/* Video pane */}
        <div className="w-full lg:w-[60%] shrink-0 flex flex-col bg-black">
          {videoId ? (
            <>
              {/* LazyYouTube handles its own aspect-ratio container */}
              <LazyYouTube
                videoId={videoId}
                title={videoLabel}
                className="w-full"
            ...
            /* No embeddable video — direct link fallback */
          ...
          {/* Topic info — mobile/tablet only */}
          <div className="px-3 sm:px-4 py-2 bg-background/90 border-t border-border lg:hidden">
            <p className="text-xs font-semibold text-foreground truncate">
              {videoLabel}
            </p>
        ...
        {/* Chat pane — stacked below on mobile/tablet, side panel on desktop */}
        <div
          className="flex-1 flex flex-col border-t lg:border-t-0 lg:border-l border-border min-h-0 overflow-hidden"
